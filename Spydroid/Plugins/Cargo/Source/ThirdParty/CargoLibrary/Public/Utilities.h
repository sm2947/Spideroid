...
namespace Kb3d {
	...
	public:
		...
		static void BuildMADSTextures(Kb3d::Assembly& assembly);
		static void BuildBaseColRgbaTextures(Kb3d::Assembly& assembly);
		...
		static void StartReceivingUnity(std::function<void(Kb3d::Assembly& assembly)> handler, std::string dcc);
		static void StartReceivingUnity(std::function<void(Kb3d::Assembly& assembly)> handler, std::string dcc, std::function<void(std::string)> logger);
	...
	private:
#if defined(KBUSDFRONT)
		static uint8_t* BuildMADSPixels(int ao_width, int ao_height, int roughness_width, int metalness_width, 
			unsigned char* ao_img, unsigned char* roughness_img, unsigned char* metalness_img, int channel_count);

		static uint8_t* BuildRGBAPixels(int texture_width, int texture_height, 
			unsigned char* basecolor_img, unsigned char* input_rgb_texture, int channel_count, bool flip);
#endif
	};
}
